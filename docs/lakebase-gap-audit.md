...
| 2 | SafeKeeper: commit = **Paxos-quorum** WAL ack; no single node's loss loses data | ✅/🟡 The safekeeper-fork quorum backend SHIPPED (roadmap §3 backend 3): `--tail-quorum` + 3× `icekeeperd` acceptors — buffered acks are 2-of-3 quorum fsyncs of SafeKeeper's own consensus algorithm (adapted with attribution), with term fencing for leader takeover; no single node's loss loses acked data, no delegated system. Functional parity with SafeKeeper's core is substantially closed; still absent vs Neon: dynamic membership/generations, acceptor S3 offload + WAL backup, acceptor-to-acceptor gossip, TLS/auth between proposer and acceptors. `--tail-dir`/`--tail-url` remain the lighter-weight classes. Sync-path durability is the Iceberg commit itself (~46–52 ms post parallel-upload), which needs no WAL tier. |
...
| 15 | Hot-row OLTP (MVCC row updates without rewrite storms) | 🟡→✅ for the upsert shape: keyed tail upserts ack exact-PK UPDATE/DELETE in 5.2 ms p50 with `--freshness-ms 25` (7.0 ms without) vs ~47.5 ms synchronous COW UPDATE, one coalesced commit per window, no `40001` storms — single-compute only. Lock choreography (`SELECT … FOR UPDATE`, queues) remains Tier-1 external Postgres territory. |
...
2. **Quorum tail residuals** (#2/#7) — the safekeeper-fork backend itself
   SHIPPED (`--tail-quorum`, see #2); still open: quorum append pipelining
   + dynamic acceptor membership.
...
**Direct answer: one planned wholesale reuse has now shipped — Neon's
safekeeper consensus core, adapted with attribution as the quorum tail
backend (`icegres/src/quorum/`, NOTICE); zero lines of Moonlink code are
vendored, forked, or linked.** Everything else shipped is icegres-native
Rust. What was actually taken, and what remains available:

| source | what we used (shipped) | what we did NOT use (open) |
|---|---|---|
| **neon** (Apache-2.0, clone at `~/neon`) | The `safekeeper/` consensus core, **forked/adapted with attribution** as the quorum `TailStore` backend (§3 backend 3, SHIPPED): the consensus state machine, term-history reconciliation, divergence truncation, and commit rule from safekeeper.rs/walproposer.c (`icegres/src/quorum/`, NOTICE). Plus concepts and specs: the branch-per-endpoint model (pre-dating this work), `wal_storage.rs`'s segment/fsync discipline as the design reference for the local tail's WAL layout, `docs/safekeeper-protocol.md` to shape the durability review attacks. | Two concrete reuse items stay open: (a) `libs/remote_storage` timeout/retry port for the object-store gap (roadmap §2.2 — STATUS: not shipped); (b) `libs/desim`-style deterministic simulation tests. |
...
Phase gates as merged: 263 tests, 173-assertion e2e,
71-assertion `tail_durability.sh` (incl. section 10's quorum proofs: all
