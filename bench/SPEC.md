...
| A10 | ODBC compatibility | `bench/clients/a10_odbc_probe.sh`: stock psqlODBC (unixODBC) connect + version/type probes, `SQLTables`/`SQLColumns` metadata, qmark-parameterized query, INSERT/readback/DELETE with rowcount (autocommit), read inside an explicit transaction — all `fail=0`. Self-contained `DRIVER=` connection string; named DSN via `infra/scripts/odbc-setup.sh` |
| A12 | Authorization (managed add-on) | `bench/clients/authz_probe.sh`: starts icegres with `--auth-file`+`--authz-file` and verifies Lakekeeper-style ReBAC — namespace-grant inheritance, table-scoped grants, warehouse ownership, role membership, per-statement SQLSTATE 42501 denial, JOIN checks every referenced table, pg_catalog/information_schema metadata always free — all `fail=0`. Gated behind the `managed` cargo feature (open-core: the OSS build errors on `--authz-file`). |
...
| A13 | flightsql-dbapi (Superset stack) | `bench/clients/a13_flightsql_dbapi_probe.py` against `icegres flight-serve`: InfluxData's DB-API 2 + SQLAlchemy dialect (the stack Superset connects through; entry point registers scheme `datafusion://` — the `+flightsql` spelling needs a manual import, see the probe's comment) — connect, schema/table/column reflection, aggregate + `SELECT * LIMIT` preview shapes, basic-auth variants against an `--auth-file` listener — all `fail=0`. Guard includes pandas (undeclared runtime dep of the driver's cursor). |
| A14 | Npgsql compatibility (Power BI driver) | `bench/clients/a14-npgsql-probe/` (.NET) against `icegres serve`: connect-time pg_type/pg_namespace type loading (the step the coherent `pg_type` patch keeps honest), typed binary reads, `SELECT *` Import-shaped scan, parameter binds, prepared reuse, `GetSchema(Tables)`, DirectQuery-shaped aggregate; two documented XFAILs: in-transaction extended SELECT (`0A000`) and `GetSchema(Columns)` (the emulation lacks `information_schema.columns.udt_schema`/`udt_name`; the probe reconnects after it) — `fail=0` otherwise. |
