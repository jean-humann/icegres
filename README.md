...
| **Arrow Flight SQL / ADBC** | Second first-class protocol: queries stream as Arrow IPC (ZSTD-compressed batches, `--result-compression`), catalog metadata (`get_objects`), prepared statements, DML, and bulk ingest (one Iceberg commit per stream). In-process TLS (`grpc+tls://`). With `--grpc-web`, the same port serves browsers directly — dashboards query the lakehouse Arrow-end-to-end via [`@icegres/flight-web`](clients/flight-web/) (direct) or [`@icegres/flight-proxy`](clients/flight-proxy/) (a named-query allowlist BFF for public apps) — measured 2.5–2.8× faster than JSON at 100k–1M rows: [`docs/frontend-dashboards.md`](docs/frontend-dashboards.md). |
...
| [`docs/clients.md`](docs/clients.md) | Copy-paste connection recipes: psql, psycopg2, SQLAlchemy, JDBC, ODBC, ADBC/Flight SQL, browser JS, BI tools. |
| [`docs/frontend-dashboards.md`](docs/frontend-dashboards.md) | Browser dashboards over Flight SQL: the measured data-path benchmark (Arrow vs JSON), `--grpc-web`, the `@icegres/flight-web` client, and the `@icegres/flight-proxy` allowlist BFF. |
...
- **Pinned dependency matrix** (do not bump independently — see `icegres/Cargo.toml`): iceberg-rust 0.10.0, DataFusion 53.1.0, arrow 58.4.0, datafusion-postgres 0.17.0 (pgwire 0.40.4), tonic 0.14, sqlparser 0.61.0, toolchain 1.96.1.
