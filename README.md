...
| **Arrow Flight SQL / ADBC** | Second first-class protocol: queries stream as Arrow IPC (ZSTD-compressed batches, `--result-compression`), catalog metadata (`get_objects`), prepared statements, DML, and bulk ingest (one Iceberg commit per stream). In-process TLS (`grpc+tls://`). With `--grpc-web`, the same port serves browsers directly — dashboards query the lakehouse Arrow-end-to-end via [`@icegres/flight-web`](clients/flight-web/) (measured 2.5–2.8× faster than JSON at 100k–1M rows: [`docs/frontend-dashboards.md`](docs/frontend-dashboards.md)). |
...
| [`docs/frontend-dashboards.md`](docs/frontend-dashboards.md) | Browser dashboards over Flight SQL: the measured data-path benchmark (Arrow vs JSON), `--grpc-web`, and the `@icegres/flight-web` client. |
